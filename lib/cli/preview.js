...
function formatMatchedTokenLines(template, comboTokens) {
  const matchedTokens = getMatchedTokens(template, comboTokens);
...
function getMatchedTokens(template, comboTokens) {
  return uniqueValues(comboTokens).filter((token) => template.tokens.has(token));
}
...
export function formatDryRunPreview({
  ...
  return [
    ...
    "",
    "Template status:",
    `  ${preview.templateStatus.message}`,
    "",
...
export function createDryRunPreview({
  ...
  return {
    ...
    matchedTokens: getMatchedTokens(template, comboTokens).map((token) => ({
      token,
      label: humanizeToken(token)
    })),
    target: {
      status: targetError ? "blocked" : "ready",
      message: targetError ?? "Ready to create"
    },
    templateStatus: {
      status: templateError ? "blocked" : "ready",
      message: templateError ?? "Ready to scaffold"
    },
    files: listTemplateFiles(template)
  };
}
