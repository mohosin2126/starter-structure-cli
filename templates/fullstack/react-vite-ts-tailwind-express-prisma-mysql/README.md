# react-vite-ts-tailwind-express-prisma-mysql

Starter scaffold for `fullstack/react-vite-ts-tailwind-express-prisma-mysql`.

## Status

This template currently ships as a structured placeholder so it can be discovered, copied, and validated safely.

## Next Steps

1. Replace placeholder files with production starter code for React + Vite + TypeScript + Tailwind + Express + Prisma + MySQL fullstack app.
2. Keep scripts and dependencies aligned with the real stack.
3. Run `npm run build:templates` and `npm run check:templates` before publishing.
